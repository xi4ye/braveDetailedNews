...
### 2026-05-10 - 多线程优化和日志系统改进

#### 新增功能 ✨
- **线程标识系统**：为每个线程添加唯一标识，提升多线程日志可读性
  - 使用线程ID后4位和新闻标题前15字符作为标识
  - 格式：`[T9952|新闻标题前15字]`
- **日志级别系统**：添加日志级别和图标
  - 🔧 TOOL: 工具调用
  - ✅ SUCCESS: 成功操作
...
#### 修复问题 🐛
- **修复ScrapyManager单例模式导致的线程冲突**
  - 问题：所有线程共享同一个 `_extractor` 实例，导致资源竞争
  - 解决：移除单例模式，每个线程创建独立实例
- **修复DeprecationWarning**
  - 问题：`twisted.internet.defer.returnValue` 已被弃用
  - 解决：改用标准 `return` 语句
...
- **黑名单机制优化**
  ...
  - 降低超时时间：从30秒降低到15秒
  - 降低重试次数：从3次降低到1次
  - 效果：失败处理时间从120秒降低到30秒
- **错误处理改进**
  - 在 `errback` 中自动记录错误到黑名单
  - 分类错误类型（超时、连接失败等）
  - 检查并提示黑名单状态
- **日志输出优化**
  - 简化线程标识（从60+字符减少到25字符）
  - 简化日志消息（去掉冗余标签）
  - 添加图标区分不同类型日志
  - 限制工具结果预览长度（100字符）
